...
class InstallmentsController extends Controller
{
    ...
    public function payByAlipay(Installment $installment)
    {
        ...
        return app('alipay')->web([
            ...
            // 这里的notify_url 和 return_url 可以覆盖掉在AppServiceProvider设置的回调地址
    ...
    public function payByWechat(Installment $installment)
    {
        ...
        if ($installment->status == Installment::STATUS_FINISHED) {
            throw new InvalidRequestException('该分期付款订单已结清');
        }
        if (!$nextItem = $installment->items()->whereNull('paid_at')->orderBy('sequence')->first()) {
            throw new InvalidRequestException('该分期付款订单已结清');
        }
        ...
        // 将生成的二维码图片数据，以字符串形式输出，并带上相应的相应类型
        ...
        $item = InstallmentItem::query()
            ->whereHas('installment', function ($query) use ($no) {
                $query->whereHas('order', function ($query) use ($no) {
                    // 根据订单表的退款流水号，找到对应的还款计划
                    $query->where('refund_no', $no);
                });
            })
            ->where('sequence', $sequence)
            ->first();
        // 没有找到对应的订单，原则上不可能发生，但为保持程序健壮性还是要加上
        if (!$item) return $failXml;
