        ...
        return [
            ...
            'sku_id' => [
                ...
                function ($attribute, $value, $fail) {
                    ...
                    // 如果是null，代表这个 SKU 不是秒杀商品
                    if (is_null($stock)) return $fail('该商品不存在');
                    // 判断库存
                    if ($stock < 1) return $fail('该商品已售完');

                    // 大多数用户在上面的逻辑里已经被拒绝了，因此下方的 SQL 查询不会对整体性能有太大影响
                    $sku = ProductSku::find($value);
                    if ($sku->product->seckill->is_before_start) return $fail('秒杀尚未开始');

                    if ($sku->product->seckill->is_after_end) return $fail('秒杀已经结束');
