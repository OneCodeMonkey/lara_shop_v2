...
class RefundInstallmentOrder implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        foreach ($installment->items as $item) {
            ...
            try {
                ...
                // 如果某个还款计划退款报错了，则暂时跳过，继续处理下一个还款计划的退款
                continue;
            }
        }
    ...
    {
        // 退款单号使用商品订单的退款号，与当前还款计划的序号，拼接而成
        ...
        switch ($item->payment_method) {
            case 'wechat':
                app('wechat_pay')->refund([
                    ...
                    // 微信支付的退款结果，并不是实时返回的，而是通过退款回调来通知，因此这里需要配上退款回调接口的地址
            ...
            case 'alipay':
                ...
                // 根据支付宝的文档，如果返回值里有 sub_code 字段，说明退款失败
                if ($ret->sub_code) {
                    $item->update([
