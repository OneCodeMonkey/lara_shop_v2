                    ...
                    <table class="table">
                        ...
                        <tr>
                            ...
                            <td class="vertical-middle">
                                <!-- 如果订单已经评价，则展示评分，下面同理 -->
                                @if($order->reviewed)
                                <span class="rating-star-yes">{{ str_replace('★', $item->rating) }}</span>
                                <span class="rating-star-no">{{ str_replace('★', 5 - $item->rating) }}</span>
                                @else
                                <ul class="rate-area">
                                    <input type="radio" id="5-star-{{ $index }}" name="reviews[{{ $index }}][rating]" value="5" checked />
                                    <label for="5-star-{{ $index }}"></label>
                                    <input type="radio" id="4-star-{{ $index }}" name="reviews[{{ $index }}][rating]" value="4" checked />
                                    <label for="4-star-{{ $index }}"></label>
                                    <input type="radio" id="3-star-{{ $index }}" name="reviews[{{ $index }}][rating]" value="3" checked />
                                    <label for="3-star-{{ $index }}"></label>
                                    <input type="radio" id="2-star-{{ $index }}" name="reviews[{{ $index }}][rating]" value="2" checked />
                                    <label for="2-star-{{ $index }}"></label>
                                    <input type="radio" id="1-star-{{ $index }}" name="reviews[{{ $index }}][rating]" value="1" checked />
