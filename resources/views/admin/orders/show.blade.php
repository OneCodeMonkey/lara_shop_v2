    ...
    <div class="box-body">
        <table class="table table-bordered">
            ...
            @if($order->ship_status === \App\Models\Order::SHIP_STATUS_PENDING)
                <!-- 判断条件 -->
                @if($order->refund_status !== \App\Models\Order::REFUND_STATUS_SUCCESS && ($order->type !== \App\Models\Order::TYPE_CROWDFUNDING || $order->items[0]->product->crowdfunding->status === \App\Models\CrowdfundingProduct::STATUS_SUCCESS))
                <tr>
                    <td colspan="4">
                        <form action="{{ route('admin.orders.ship', [$order->id]) }}" method="post" class="form-inline">
                            <!-- 别忘了加上 @csrf token 关键词 -->
                            {{ csrf_field() }}
...
<script>
    $(document).ready(function () {
        ...
        $('#btn-refund-disagree').click(function () {
            // Laravel-Admin 使用的 SweetAlert 版本于我们在前台使用的版本不一样，因此参数也不太一样
            swal({
                title: '输入拒绝退款理由',
                input: 'text',
                ...
                preConfirm : function (inputValue) {
                    ...
                    // Laravel-Admin 没有 axios，用jQuery的ajax做请求
                    return $.ajax({
